...
void EmitSillhouette(vec3 v0, vec3 v1, vec3 v0_dir, vec3 v1_dir) {
	...
	EmitVertex();
	gl_Position = ProjectionViewMatrix * vec4(v1 - v1_dir * 0.0001, 1.0);
	EmitVertex();
...
void main() {
	...
	const vec3 v0_side = gl_in[1].gl_Position.xyz;
	const vec3 v1_side = gl_in[3].gl_Position.xyz;
	const vec3 v2_side = gl_in[5].gl_Position.xyz;
	...
	if (dot(main_normal, light_dir0) < 0.0) {
		...
		// Front cap
		gl_Position = ProjectionViewMatrix * vec4(v0 - light_dir0 * 0.0001, 1.0);
		EmitVertex();
		gl_Position = ProjectionViewMatrix * vec4(v1 - light_dir1 * 0.0001, 1.0);
		EmitVertex();
		gl_Position = ProjectionViewMatrix * vec4(v2 - light_dir2 * 0.0001, 1.0);
		EmitVertex();
		EndPrimitive();
