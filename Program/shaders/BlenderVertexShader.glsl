...
void main(){
	mat4 mesh_model_matrix = model_matrix * mesh_matrix;

	gl_Position = projection_matrix * view_matrix * mesh_model_matrix * vec4(vertex_position, 1);
	
	vertex_position_worldspace = (mesh_model_matrix * vec4(vertex_position,1)).xyz;
	
	vec3 vertex_position_cameraspace = ( view_matrix * mesh_model_matrix * vec4(vertex_position,1)).xyz;
	...
	normal_cameraspace = ( view_matrix * mesh_model_matrix * vec4(vertex_normal,0)).xyz;
	UV = vertex_uv;
